...
class FlowControl(DataIngestionImpl):
                    ...
                    if self.common.checkJsonKeyExists(cfg, "hive"):
                        if self.app_createhivetable:
                            self.buildCreateHiveTableRequest(key=key, key2=key2,
                                ...
                                partition_fields=self.common.getJsonValue(ingestion,"hive","partitions"),
    ...
    # Servizio http
    def buildReadDataRequest(self, key=None,key2=None, url=None, type=None, method=None, filename=None, payload=None,timeout=None,user=None,password=None):
    ...
    def buildCreateHiveTableRequest(self,key=None,table=None, db=None, schema=None, mode=None,location=None,partition_fields=None,file_format=None):
        key = key if key is not None else -1
    ...
    def buildWriteDataToDbRequest(self,key=None,key2=None,spark=False,type=None,host=None,port=None,service=None,database=None,schema=None,user=None,password=None,table=None,mode=None):
        ...
        spark = spark if spark is not None else self.getParam("common","service","ingestion","db","spark")
        ...
        assert table is not None, "buildWriteDataToDbRequest - 'schema' not set in configuration file"
    ...
    def unionDataframes(self, buildDataFrameResponseList, limit_df_size=None):
        ...
        union_df_hive, union_df_oracle, union_df_csv = None, None, None
        union_df_hive_list, union_df_oracle_list, union_df_csv_list = [], [], []
        ...
        try:
            ...
            for responseList in buildDataFrameResponseList:
                for buildDataFrameResponse in responseList:
                    ...
                    df_oracle = buildDataFrameResponse.df_oracle()
                    ...
                    if df_oracle is not None:
                        union_df_oracle = union_df_oracle.union(df_oracle) if union_df_oracle is not None else df_oracle
                        if limit_df_size is not None and (union_df_oracle.count() > limit_df_size or check_last_response):
                            union_df_oracle_list.append(union_df_oracle)
                            union_df_oracle = None
            ...
            if limit_df_size is None:
                ...
                union_df_oracle_list.append(union_df_oracle) if union_df_oracle is not None else None
            ...
            if len(union_df_oracle_list) > 0:
                self.logger.info("unionDataframes [{}] - total union_df_oracle: {}".format(self.app_name, len(union_df_oracle_list)))
                self.writeDataToDbRequestList = []
                for df in union_df_oracle_list:
                    self.logger.info("unionDataframes [{}] - union_df_oracle count: {}".format(self.app_name, df.count()))
                                ...
                                if buildDataFrameResponse2.df_hive() is not None :
                                    for writeDataToHiveRequest in self.writeDataToHiveRequestList:
                                        if writeDataToHiveRequest.key() == buildDataFrameResponse2.key()\
                                            ...
                                            writeDataToHiveRequest.data( buildDataFrameResponse2.df_hive() )
                                ...
                                if buildDataFrameResponse2.df_db() is not None :
                                    for writeDataToDbRequest in self.writeDataToDbRequestList:
                                        if writeDataToDbRequest.key() == buildDataFrameResponse2.key()\
                                            ...
                                            writeDataToDbRequest.data( buildDataFrameResponse2.df_db() )
