...
class DataIngestionImpl(DataIngestionInterface):
    ...
    def initDatabase(self, remote_log=None):
        try:
            ...
            if self.app_db_logger:
                ...
                self.dbLogger = DbLogger(self.dbClient, self.db_log_table, run_id, self.app_name, self.app_description,
                                         remote_log) if remote_log and self.enableOperations else None
    ...
    def readDataImpl(self, readDataRequestObj: ReadDataRequest):
        try:
            # response
            ...
            if request_type.upper() == RequestType.SOAP:
                session = Session()
                ...
                if protocol.upper() == ProtocolType.SSH:
                    ...
                    # elif request_type.upper() == RequestType.FILE:
                        ...
                        data_ = self.getSshClient().getFileContentSFTP(file_remote)
                        filename_ = file_remote
                    ...
                    if request_type.upper() == RequestType.FOLDER:
                        ...
                        try:
                            name_and_content_file_list = self.getFtpClient().retriveFilenameAndBinaryFilesFromFolder(
                        ...
                        data_ = list(self.getFtpClient().retriveBinaryFile(file_remote))
                        filename_ = file_remote
    ...
    def writeDataToCSV(self, writeDataToCSVRequestList) -> NoReturn:
    ...
    def writeDataToCSVImpl(self, writeDataToCSVRequestObj) -> NoReturn:
        try:
    ...
    def writeDataToHdfsImpl(self, writeDataToHdfsRequestObj):
        try:
            ...
            if writeDataToHdfsRequestObj._data is None:
                self.logger.warning("writeDataToHdfsImpl [{}] - data is None".format(k))
                return

            self.logger.info("writeDataToHdfsImpl [{}] - len(writeDataToHdfsRequestObj._data): {}".format(k, len(
                writeDataToHdfsRequestObj._data)))
            data_list = self.common.toList(writeDataToHdfsRequestObj._data)
            filename_list = self.common.toList(writeDataToHdfsRequestObj._filename)
            append_ = False
            overwrite_ = False

            self.logger.info("writeDataToHdfsImpl [{}] - len(data_list): {}".format(k, len(data_list)))
            self.logger.info("writeDataToHdfsImpl [{}] - len(filename_list): {}".format(k, len(filename_list)))

            for i in range(len(data_list)):
                # data = writeDataToHdfsRequestObj._data
                self.logger.info("writeDataToHdfsImpl [{}] - i: {}".format(k, i))
                self.logger.info("writeDataToHdfsImpl [{}] - filename_list[i]: {}".format(k, filename_list[i]))
                data = data_list[i]
            ...
            if not check_final_table_exist:
                ...
                if self.enableOperations:
                    df_final.write \
                        ...
                        .partitionBy(*createHiveTableRequestObj._partition_fields) \
    ...
    def endOK(self):
        ...
        self.dbLogger.logEndExecution(self.exit_code) if (self.enableOperations and self.app_db_logger) else None
