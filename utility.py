...
class Http:
    def request(self, url, type, method, payload=None, timeout=40):
...
class DbLogger():
    ...
    def __init__(self, db, log_table, run_id, script_name, description="", log_path="", app_id=""):
        self.db = db
    ...
    def logEndExecution(self, status=0, description=None):
        ...
        if description is not None:
            query += "DESCRIPTION='{}', ".format( description.format( "'", "''" ) )
        ...
        try:
            ...
            if self.db_type == self.DB_TYPE_ORACLE:
                q = "MERGE INTO {table} d USING (select 1 FROM DUAL) \
                    ...
                    table=table_name, script=script, key=key, value=value, type=type)
...
class Spark:
    ...
    #Modifica Michele
    def getDfFromQueryOld(self, query, oracle=False,oracle_dict=None):
        ''' 
            oracle_dict["dbtable"] deve essere o la tabella o la query con l'alias finale esempio:
                 (select * from te_viaggi where ROWNUM <2) te_viaggi
            e senza campi geometrici nella select
        '''
        if oracle and oracle_dict is not None:
            print(repr(oracle_dict))
            return self.spark.read \
            .format("jdbc") \
            .option("url",oracle_dict["url"] ) \
            .option("dbtable", oracle_dict["dbtable"]) \
            .option("user", oracle_dict["user"]) \
            .option("password", oracle_dict["password"]) \
            .option("driver", "oracle.jdbc.driver.OracleDriver") \
            .load()
...
class SqlAlchemy:
    def __init__(self,base, allClasses, db_url,create=False,delete_all=False,verbose=False, schema=None):
        ...
        try:
            ...
            logger.error("Errore initializeSessionDb ", e)
        ...
        try:
            for table in self.base.metadata.tables.keys():
                if not self.engine.has_table( table ):
                    if create:
                        logger.info( "Creazione tabella {}".format( table ) )
                        self.base.metadata.tables[table].create( self.engine )
                    else:
                        logger.info( "Tabella assente {}}!".format( table ) )
                        exit( 1 )
            return True
        except Exception as e:
            logger.error("Errore checkTableExist ", e)
            return False
...
class ExceptionManager:
    ...
    def handle_uncaught_exception(self, exc_type, exc_value, exc_traceback):
        ...
        print("ERROR")
        print("ExceptionManager - handle_uncaught_exception - {}".format(dict(exc_type=exc_type, exc_value=exc_value, exc_traceback=exc_traceback)))
        print("ERROR")
        self.uncaught_exceptions.append(dict(exc_type=exc_type, exc_value=exc_value, exc_traceback=exc_traceback))

    def __str__(self):
        return str(dict(exceptions_occurred=[str(exc) for exc in self.exceptions_occurred], uncaught_exceptions=[str(exc) for exc in self.uncaught_exceptions]))
